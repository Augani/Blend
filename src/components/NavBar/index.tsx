...
function index({ className, children }: Props): ReactElement {
  return (
    <nav
      className={`w-full h-24 flex flex-row justify-between items-center px-10 ${className}`}
    >
      <a
        href="/"
        className="tracking-widest no-underline text-gray-800 font-bold text-3xl uppercase"
      >
        Blend
      </a>
      <div className="h-full flex-row flex items-center">
       ...
       <svg
        ...
        <p className="font-semibold text-lg text-center text-gray-50">Empty</p>
       </Button>
      </div>
    </nav>
  );
